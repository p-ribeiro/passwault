...
def handle_imagepass(args, session_manager):
    ...
    if args.option == "encode":
        return embedder.encode(image_path=args.image_path, password=args.password, session_manager=session_manager)
    else:
...
def cli(ctx: AppContext):
    try:
        ...
        register_parser.set_defaults(func=lambda args: register(args.username, args.password, args.role, ctx))
        ...
        login_parser.set_defaults(func=lambda args: login(args.username, args.password, ctx))
